package com.example.imagecropdrawandtexteditor.editimage.view.imagezoom

import androidx.appcompat.widget.AppCompatImageView
import com.example.imagecropdrawandtexteditor.editimage.view.imagezoom.utils.IDisposable
import android.graphics.drawable.Drawable
import com.example.imagecropdrawandtexteditor.editimage.view.imagezoom.easing.Easing
import com.example.imagecropdrawandtexteditor.editimage.view.imagezoom.easing.Cubic
import android.graphics.PointF
import android.graphics.RectF

import android.graphics.Bitmap
import com.example.imagecropdrawandtexteditor.editimage.view.imagezoom.graphic.FastBitmapDrawable
...
abstract class ImageViewTouchBase : AppCompatImageView, IDisposable {
    ...
    protected var mEasing = Cubic() as Easing
    ...
    override fun onLayout(
        ...
        if (drawable != null) {
            if (changed || mScaleTypeChanged || mBitmapChanged) {
                ...
                if (changed || mBitmapChanged || mScaleTypeChanged) onLayoutChanged(left,
                    top,
                    right,
                    bottom)
    ...
    protected open fun _setImageDrawable(
        ...
        if (drawable != null) {
            if (LOG_ENABLED) {
                Log.d(LOG_TAG, "size: " + drawable.intrinsicWidth + "x"
    ...
    protected fun updateRect(bitmapRect: RectF?, scrollRect: RectF) {
        ...
        if (bitmapRect.left >= 0 && bitmapRect.right <= mThisWidth) scrollRect.left = 0f
        ...
        if (bitmapRect.left + scrollRect.left >= 0) scrollRect.left =
            (0 - bitmapRect.left).toInt().toFloat()
        if (bitmapRect.right + scrollRect.left <= mThisWidth - 0) scrollRect.left =
            (mThisWidth - 0 - bitmapRect.right).toInt().toFloat()
    }
        ...
        mHandler.post(object : Runnable {
            ...
            override fun run() {
                ...
                if (currentMs < durationMs) {
                    ...
                    if (centerRect.left != 0f || centerRect.top != 0f) scrollBy(centerRect.left,
                        centerRect.top)
                }
            }
        })
    ...
    protected fun zoomTo(
        ...
        mHandler.post(object : Runnable {
            override fun run() {
                ...
                val newScale = mEasing.easeInOut(currentMs.toDouble(), 0.0,
                    deltaScale.toDouble(), durationMs.toDouble()).toFloat()
    ...
    companion object {
        const val LOG_TAG = "ImageViewTouchBase"
